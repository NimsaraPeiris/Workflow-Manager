...
export const EditPermissionsModal = ({
    ...
    // Check if anything changed from original user data (must be before any early returns - Rules of Hooks)
    const isDirty = !!(user && editData && (
        editData.fullName !== user.full_name ||
        editData.departmentId !== user.department_id ||
        editData.teamId !== (user.team_id || '') ||
        editData.roleId !== (user.role_id || '') ||
        JSON.stringify([...(editData.permissions || [])].sort()) !== JSON.stringify([...(Array.isArray(user.permissions) ? user.permissions : [])].sort())
    ));
                            ...
                            <form onSubmit={handleSubmit} className="flex flex-col h-full overflow-hidden">
                                <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between items-center shrink-0">
                                    ...
                                    <div className="flex items-center gap-6">
                                        ...
                                        <button onClick={handleClose} type="button" className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-none transition-colors">
